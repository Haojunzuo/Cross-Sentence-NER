...
class Bert_LSTM_NerModel(nn.Module):
    ...
    def forward(self, batch_index, sentence_index, batch_label=None, batch_type=None):
        ...
        if batch_type == 'train':
            newTensor = self.train_wwm_unsupervised[start - self.windowSize: end - self.windowSize].reshape(cur_batch_size, 1, 768)
            newTensor = torch.cat([newTensor, self.train_glyce_unsupervised[start - self.windowSize: end - self.windowSize].reshape(cur_batch_size, 1, 768)], dim=1)
            for i in reversed(range(self.windowSize-1)):
                tempLast = self.train_wwm_unsupervised[start - i-1: end - i-1].reshape(cur_batch_size, 1, 768)
                newTensor = torch.cat([newTensor, tempLast], dim=1)
                tempLast = self.train_glyce_unsupervised[start - i - 1: end - i - 1].reshape(cur_batch_size, 1, 768)
                newTensor = torch.cat([newTensor, tempLast], dim=1)
            newTensor = torch.cat([newTensor, bert_out0], dim=1)
            for i in range(self.windowSize):
                tempNext = self.train_wwm_unsupervised[start + i+1: end + i+1].reshape(cur_batch_size, 1, 768)
                newTensor = torch.cat([newTensor, tempNext], dim=1)
                tempNext = self.train_glyce_unsupervised[start + i + 1: end + i + 1].reshape(cur_batch_size, 1, 768)
                newTensor = torch.cat([newTensor, tempNext], dim=1)

        if batch_type == 'dev':
            newTensor = self.dev_wwm_unsupervised[start - self.windowSize: end - self.windowSize].reshape(cur_batch_size, 1, 768)
            newTensor = torch.cat([newTensor, self.dev_glyce_unsupervised[start - self.windowSize: end - self.windowSize].reshape(cur_batch_size, 1, 768)], dim=1)
            for i in reversed(range(self.windowSize-1)):
                tempLast = self.dev_wwm_unsupervised[start - i - 1: end - i - 1].reshape(cur_batch_size, 1, 768)
                newTensor = torch.cat([newTensor, tempLast], dim=1)
                tempLast = self.dev_glyce_unsupervised[start - i - 1: end - i - 1].reshape(cur_batch_size, 1, 768)
                newTensor = torch.cat([newTensor, tempLast], dim=1)
            newTensor = torch.cat([newTensor, bert_out0], dim=1)
            for i in range(self.windowSize):
                tempNext = self.dev_wwm_unsupervised[start + i + 1: end + i + 1].reshape(cur_batch_size, 1, 768)
                newTensor = torch.cat([newTensor, tempNext], dim=1)
                tempNext = self.dev_glyce_unsupervised[start + i + 1: end + i + 1].reshape(cur_batch_size, 1, 768)
                newTensor = torch.cat([newTensor, tempNext], dim=1)
