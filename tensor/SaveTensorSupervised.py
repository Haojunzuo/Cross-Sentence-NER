...
class BertDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        attention_mask = self.tokenizer.encode_plus(text, add_special_tokens=True, max_length=self.max_len + 2,
                                           padding="max_length", truncation=True, return_tensors="pt").attention_mask
        label_index = torch.tensor(label_index)
        return text_index.reshape(-1), label_index, len(label), attention_mask
...
class Bert_NerModel(nn.Module):
    ...
    def forward(self, batch_index, attention_mask, batch_label=None, saveTensor=False):
        ...
        star_out, relay = self.starTransformer(bert_out0, attention_mask)

        if saveTensor == True:
            self.relayTensor = torch.cat([self.relayTensor, relay], 0)
            if shape != 50:
                torch.save(self.relayTensor, 'relayTensorDevSupervised.pt')
        # relayNumpy = np.array(relay.cpu().detach().numpy())
        # self.nmpArray = np.concatenate((self.nmpArray, relayNumpy), axis=0)
        # if shape != 50 and saveTensor==True:
        #     torch.save(self.relayTensor, 'relayTensorTrain.pt')
...
# 有监督学习，训练50次，进行梯度下降，保存中继节点向量到pt文件
if __name__ == "__main__":
    relayTensor = torch.load('relayTensorDevSupervised.pt')
    ...
    for e in range(epoch):
        model.train()
        for batch_text_index, batch_label_index, batch_len, attention_mask in dev_dataloader:
            batch_text_index = batch_text_index.to(device)
            batch_label_index = batch_label_index.to(device)
            attention_mask = attention_mask.reshape(attention_mask.shape[0],32)
            attention_mask = attention_mask.to(device)

            if e == 49:
                loss = model.forward(batch_index=batch_text_index, batch_label=batch_label_index,
                                 attention_mask=attention_mask, saveTensor=True)
            else:
                loss = model.forward(batch_index=batch_text_index, batch_label=batch_label_index,
